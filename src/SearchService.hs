...
import Search_Types
import qualified SearchService_Iface as Iface
...
from_Search_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v61 -> P.Just (1, ("query",from_SearchQuery _v61))) $ search_args_query record
  ]
...
to_Search_args (T.TStruct fields) = Search_args{
  search_args_query = P.maybe (search_args_query default_Search_args) (\(_,_val63) -> (case _val63 of {T.TStruct _val64 -> (to_SearchQuery (T.TStruct _val64)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
...
from_Search_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v67 -> (1, ("ex",Services_Types.from_ServicesException _v67))) <$> search_result_ex record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v67 -> P.Just (0, ("success",from_SearchResult _v67))) $ search_result_success record
    , (\_v67 -> (1, ("ex",Services_Types.from_ServicesException _v67))) <$> search_result_ex record
    ]
    )
...
to_Search_result (T.TStruct fields) = Search_result{
  search_result_success = P.maybe (search_result_success default_Search_result) (\(_,_val69) -> (case _val69 of {T.TStruct _val70 -> (to_SearchResult (T.TStruct _val70)); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  search_result_ex = P.maybe (P.Nothing) (\(_,_val69) -> P.Just (case _val69 of {T.TStruct _val71 -> (Services_Types.to_ServicesException (T.TStruct _val71)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
...
from_GetCapabilities_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v79 -> (1, ("ex",Services_Types.from_ServicesException _v79))) <$> getCapabilities_result_ex record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v79 -> P.Just (0, ("success",T.TList (T.T_STRUCT typemap_SearchCapability) $ P.map (\_v81 -> from_SearchCapability _v81) $ Vector.toList _v79))) $ getCapabilities_result_success record
    , (\_v79 -> (1, ("ex",Services_Types.from_ServicesException _v79))) <$> getCapabilities_result_ex record
    ]
    )
...
to_GetCapabilities_result (T.TStruct fields) = GetCapabilities_result{
  getCapabilities_result_success = P.maybe (getCapabilities_result_success default_GetCapabilities_result) (\(_,_val83) -> (case _val83 of {T.TList _ _val84 -> (Vector.fromList $ P.map (\_v85 -> (case _v85 of {T.TStruct _val86 -> (to_SearchCapability (T.TStruct _val86)); _ -> P.error "wrong type"})) _val84); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  getCapabilities_result_ex = P.maybe (P.Nothing) (\(_,_val83) -> P.Just (case _val83 of {T.TStruct _val87 -> (Services_Types.to_ServicesException (T.TStruct _val87)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
...
from_GetCorpora_result record = T.TStruct $ Map.fromList 
  (let exns = M.catMaybes [ (\_v95 -> (1, ("ex",Services_Types.from_ServicesException _v95))) <$> getCorpora_result_ex record]
  in if P.not (P.null exns) then exns else M.catMaybes
    [ (\_v95 -> P.Just (0, ("success",T.TList T.T_STRING $ P.map (\_v97 -> T.TString $ E.encodeUtf8 _v97) $ Vector.toList _v95))) $ getCorpora_result_success record
    , (\_v95 -> (1, ("ex",Services_Types.from_ServicesException _v95))) <$> getCorpora_result_ex record
    ]
    )
...
to_GetCorpora_result (T.TStruct fields) = GetCorpora_result{
  getCorpora_result_success = P.maybe (getCorpora_result_success default_GetCorpora_result) (\(_,_val99) -> (case _val99 of {T.TList _ _val100 -> (Vector.fromList $ P.map (\_v101 -> (case _v101 of {T.TString _val102 -> E.decodeUtf8 _val102; _ -> P.error "wrong type"})) _val100); _ -> P.error "wrong type"})) (Map.lookup (0) fields),
  getCorpora_result_ex = P.maybe (P.Nothing) (\(_,_val99) -> P.Just (case _val99 of {T.TStruct _val103 -> (Services_Types.to_ServicesException (T.TStruct _val103)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
