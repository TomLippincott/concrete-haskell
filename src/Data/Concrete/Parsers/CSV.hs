...
-- fileParser :: Maybe [Text] -> Char -> CommunicationParser [Communication]
-- fileParser h d = do
--   fs <- if h == Nothing then header d else return $ fromJust h
--   space
--   cs <- (parser fs d) `sepEndBy` newline --withFields fs d
--   space
--   eof
--   return cs

-- --withFields :: [Text] -> Char -> CommunicationParser Communication
-- --withFields fs d = (communicationRule id (row d fs)) `sepEndBy` (newline)

-- parser :: [Text] -> Char -> CommunicationParser Communication
-- parser fs d = communicationRule id (row d fs)
